...
var TSPAutomatorSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    }, "Folder Path (e.g. 01 Projects)", "Index Name (e.g. Projects Index)", "Folder Path", "Index Filename");
    ...
    this.renderMapSettings(containerEl, "Folder Template Map", "Map folders to the template applied to new files inside them.", this.plugin.settings.folderTemplateMap, async (newMap) => {
      ...
      await this.plugin.saveSettings();
    }, "Folder Path (e.g. 01 Projects)", "Template Path (e.g. Templates/Note.md)", "Folder Path", "Template Path");
  }
  renderMapSettings(containerEl, heading, desc, data, onSave, keyPlaceholder, valuePlaceholder, keyLabel, valueLabel) {
    ...
    entries.forEach(([key, value], index) => {
      ...
      const keyInput = new import_obsidian.TextComponent(setting.infoEl).setPlaceholder(keyPlaceholder).setValue(key).onChange(async (newKey) => {
